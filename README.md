...
```bash
git checkout dev          # 지금 작업 브랜치를 dev로 전환 (로컬 파일 내용도 dev 상태로 바뀜)
git pull origin dev       # GitHub의 dev 최신 내용을 내 컴퓨터로 받아오기
```
...
# --base dev: "이 브랜치 내용을 dev에 합쳐달라"는 뜻 (합쳐질 대상 브랜치)
...
### 7. 머지 후 로컬 정리
```bash
git checkout dev                                 # dev 브랜치로 돌아오기
